...
output_folder = os.path.join(current_dir, 'images', 'Task1bis')
...
print("Generazione Initial Guess tramite rollout dinamico...")
for tt in range(TT - 1):
    # dynamics_euler restituisce [x_next, A, B], prendiamo solo x_next ([0])
...
for kk in range(max_iters):
    ...
    # ---------------------------------------------------------
    iters_to_debug = [0, 15] # iteration samples of which we'll plot armijo
...
# STOP CONDITION
    if gamma < 1e-7:
        print(f"Optimization stalled (Step too small) at iter {kk}. Treating as converged.")
        max_iters = kk
        break
...
if len(du_history) > 0:
    ...
    for i in range(len(du_history)):
        ...
        # CALCOLO QUADRATICO (Come il tuo amico)
        norm_sq = np.sum(du_traj**2)
        descent_norms.append(norm_sq)
        iterations.append(i)

    plt.figure(figsize=(10, 6))
    # 'bo-' crea i pallini blu collegati
    plt.semilogy(iterations, descent_norms, 'bo-', linewidth=2, markersize=6, label=r'$||\Delta u||^2$')
    
    # TITOLO AGGIORNATO (Se non vedi questo titolo, il plot non si è aggiornato!)
